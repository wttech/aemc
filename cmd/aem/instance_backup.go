	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for _, instance := range instances {
				...
				if !local.IsCreated() {
					log.Infof("instance '%s': skipping making backup as it is not created", instance.ID())
					continue
				}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for _, instance := range instances {
				local := instance.Local()
				if local.IsCreated() {
					log.Infof("instance '%s': skipping using backup as it is already created", instance.ID())
					continue
				}
				file, err := local.ProposeBackupFileToUse()
				if err != nil {
					log.Warnf("instance '%s': skipping using backup as cannot find any: %s", instance.ID(), err)
					continue
				}
				running := local.IsRunning()
				if running {
