...
func (r Repo) Save(path string, props map[string]any) error {
	log.Infof("%s > saving node '%s'", InstanceId(r.instance), path)
	resp, err := r.requestFormData("", props).Post(path)
	if err := r.handleResponse(fmt.Sprintf("%s > cannot save node '%s'", InstanceId(r.instance), path), resp, err); err != nil {
		return err
	}
	log.Infof("%s > saved node '%s'", InstanceId(r.instance), path)
	return nil
}

func (r Repo) Delete(path string) error {
	log.Infof("%s > deleting node '%s'", InstanceId(r.instance), path)
	resp, err := r.requestFormData("delete", map[string]any{}).Post(path)
	if err = r.handleResponse(fmt.Sprintf("%s > cannot delete node '%s'", InstanceId(r.instance), path), resp, err); err != nil {
		return err
	}
	log.Infof("%s > deleted node '%s'", InstanceId(r.instance), path)
	return nil
}

func (r Repo) Copy(sourcePath string, targetPath string) error {
	log.Infof("%s > copying node from '%s' to '%s'", InstanceId(r.instance), sourcePath, targetPath)
	resp, err := r.requestFormData("copy", map[string]any{":dest": targetPath}).Post(sourcePath)
	if err = r.handleResponse(fmt.Sprintf("%s > cannot copy node from '%s' to '%s'", InstanceId(r.instance), sourcePath, targetPath), resp, err); err != nil {
		return err
	}
	log.Infof("%s > copied node from '%s' to '%s'", InstanceId(r.instance), sourcePath, targetPath)
	return nil
}

func (r Repo) Move(sourcePath string, targetPath string, replace bool) error {
	log.Infof("%s > moving node from '%s' to '%s'", InstanceId(r.instance), sourcePath, targetPath)
	resp, err := r.requestFormData("move", map[string]any{":dest": targetPath, ":replace": replace}).Post(sourcePath)
	if err = r.handleResponse(fmt.Sprintf("%s > cannot move node from '%s' to '%s'", InstanceId(r.instance), sourcePath, targetPath), resp, err); err != nil {
		return err
	}
	log.Infof("%s > moved node from '%s' to '%s'", InstanceId(r.instance), sourcePath, targetPath)
	return nil
}
...
func (r Repo) Download(remotePath string, localFile string) error {
	log.Infof("%s > downloading node '%s'", InstanceId(r.instance), remotePath)
	if err := httpx.DownloadWithOpts(httpx.DownloadOpts{
		...
		return fmt.Errorf("%s > cannot download node '%s': %w", InstanceId(r.instance), remotePath, err)
	}
	log.Infof("%s > downloaded node '%s'", InstanceId(r.instance), remotePath)
	return nil
}
