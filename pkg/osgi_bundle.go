...
func (b OSGiBundle) assumeExists() (*OSGiBundleState, error) {
	...
	if !state.Exists {
		return state, fmt.Errorf("bundle '%s' does not exist on instance '%s'", b.symbolicName, b.manager.instance.ID())
	}
	return state, nil
}
...
func (b OSGiBundle) Await(state string, condition func() bool, timeout time.Duration) error {
	...
	for {
		...
		log.Infof("instance '%s' is awaiting bundle '%s' state '%s'", b.manager.instance.ID(), b.symbolicName, state)
