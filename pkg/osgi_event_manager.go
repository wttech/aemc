...
func (em *OSGiEventManager) List() (*osgi.EventList, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("instance '%s': cannot request event list: %w", em.instance.ID(), err)
	} else if resp.IsError() {
		return nil, fmt.Errorf("instance '%s': cannot request event list: %s", em.instance.ID(), resp.Status())
	}
	var res = new(osgi.EventList)
	if err = fmtx.UnmarshalJSON(resp.RawBody(), res); err != nil {
		return nil, fmt.Errorf("instance '%s': cannot parse event list response: %w", em.instance.ID(), err)
	}
	return res, nil
}
