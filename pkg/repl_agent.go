...
func (ra ReplAgent) Setup(props map[string]any) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("instance '%s': cannot read replication agent '%s': %w", ra.instanceID(), ra.page.Path(), err)
	}
	if !pageState.Exists {
		...
		if err != nil {
			return false, fmt.Errorf("instance '%s': cannot setup replication agent '%s': %w", ra.instanceID(), ra.page.Path(), err)
		}
		changed = true
	}
	...
	if err != nil {
		return changed, fmt.Errorf("instance '%s': cannot read replication agent '%s' exist: %w", ra.instanceID(), pageContent.Path(), err)
	}
	if !pageContentState.Exists {
		...
		if err != nil {
			return changed, fmt.Errorf("instance '%s': cannot create replication agent '%s': %w", ra.instanceID(), pageContent.Path(), err)
		}
		changed = true
	} else {
		changed, err = pageContent.SaveWithChanged(props) // TODO react when transportPassword changes externally, now is ignored
		if err != nil {
			return changed, fmt.Errorf("instance '%s': cannot update replication agent '%s': %w", ra.instanceID(), pageContent.Path(), err)
		}
	}
...
func (ra ReplAgent) Delete() (bool, error) {
	...
	err = ra.page.Delete()
	if err != nil {
		return false, fmt.Errorf("instance '%s': cannot delete replication agent '%s': %w", ra.instanceID(), ra.page.Path(), err)
	}
	return true, nil
}
