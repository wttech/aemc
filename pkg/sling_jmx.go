...
func (j JMX) ReadBean(name string, out interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("%s > cannot read Sling JMX Bean '%s': %w", j.instance.IDColor(), name, err)
	} else if response.IsError() {
		return fmt.Errorf("%s > cannot read Sling JMX Bean '%s': %s", j.instance.IDColor(), name, response.Status())
	}
	if err := fmtx.UnmarshalJSON(response.RawBody(), &out); err != nil {
		return fmt.Errorf("%s > cannot read Sling JMX Bean '%s'; cannot parse response: %w", j.instance.IDColor(), name, err)
	}
	return nil
}
